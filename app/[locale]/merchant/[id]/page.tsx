...
export default function MerchantFallback({ params }: { params: Promise<{ id: string }> }) {
  const unwrappedParams = use(params);
  const [attemptedDeepLink, setAttemptedDeepLink] = useState(false);
  const [showFallback, setShowFallback] = useState(false);
  const deepLinkUrl = `lait://merchant/${unwrappedParams.id}`;
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-20 mt-16 text-center">
        <div className="max-w-md w-full flex flex-col items-center gap-6">
          ...
          <h1 className="text-3xl font-bold">Merchant Partner</h1>
          <p className="text-base text-muted-foreground">
            Explore exclusive deals, rewards, and locations for this merchant partner directly on the LAIT app.
          </p>
          ...
          {(showFallback || true) && (
            <div className="w-full bg-foreground/5 rounded-2xl p-6 mt-4 border border-foreground/10">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                 If the app didn't open automatically, tap the button below to browse offers and discover new places.
              </p>
